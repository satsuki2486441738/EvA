# kimia_infer/models/tokenizer/ced_base/modeling_ced.py
# coding=utf-8
...
class BN_FP32(nn.BatchNorm2d):
    def _cast_state_fp32(self):
        ...
        if self.running_mean.dtype != torch.float32:
            self.running_mean.data = self.running_mean.data.float()
            self.running_var.data  = self.running_var.data.float()

    def _apply(self, fn):
        super()._apply(fn)
        self._cast_state_fp32()
        return self
...
class CedModel(CedPreTrainedModel):
    def __init__(self, config: CedConfig) -> None:
        ...
        self.init_bn = BN_FP32(config.n_mels, momentum=0.01)
        ...
        act_layer = nn.GELU
        dpr = torch.linspace(0, config.drop_path_rate, config.depth, device='cpu').tolist()  # stochastic depth decay rule
    ...
    def forward_features(self, x: torch.Tensor, output_hidden_states=False):
        ...
        x = self.norm(x)
        return (x, hidden_states) if output_hidden_states else x

    def forward(self, input_values: torch.Tensor, output_hidden_states=False):
        r"""
        Runs a forward pass of the CED model as an audio encoder.
        """

        x = torch.unsqueeze(input_values, 1)  # Adding channel dimension
        x = torch.permute(x, (0, 2, 1, 3))   # Change to shape (batch_size, 1, height, width)
        x = self.init_bn(x)  # Apply batch normalization
        x = torch.permute(x, (0, 2, 1, 3))  # Revert permutation
        ...
        if x.shape[-1] > self.maximal_allowed_length:
            parts = list(x.split(self.maximal_allowed_length, dim=-1))  # list of [B,C,F,t_i]
            if parts[-1].shape[-1] < self.maximal_allowed_length:
                if self.config.pad_last:
                    pad = x.new_zeros(*x.shape[:-1], self.maximal_allowed_length)
                    pad[..., : parts[-1].shape[-1]] = parts[-1]
                    parts[-1] = pad
                else:
                    parts = parts[:-1]  # 仅在明确不补零时才丢尾块

            splits = torch.stack(parts, dim=0)
            n_splits = len(splits)
        ...
        if output_hidden_states:
            ...
            x, hidden_states = self.forward_features(x, output_hidden_states=True)
        else:
            x = self.forward_features(x)

        x = torch.reshape(x, (x.shape[0] // n_splits, -1, x.shape[-1]))
        #print(f"ced forword successfully")
        if output_hidden_states:
            # 也reshape每层的hidden_states
            hidden_states = [
                torch.reshape(h, (h.shape[0] // n_splits, -1, h.shape[-1]))
                for h in hidden_states
            ]
            return SequenceClassifierOutput(logits=x, hidden_states=hidden_states)
        else:
...
class CedForAudioClassification(CedPreTrainedModel):
    ...
    def forward(self, input_values: torch.Tensor, labels: Optional[torch.Tensor] = None, output_hidden_states=False):
        """
        ...
        ```
        """
        last_hidden_states = self.encoder(input_values, output_hidden_states=output_hidden_states)

        if isinstance(last_hidden_states, SequenceClassifierOutput):
            features = last_hidden_states.logits
            hidden_states = last_hidden_states.hidden_states
        else:
...
class CedEncoder(nn.Module):
    def __init__(self, model_name_or_path: str):
        super().__init__()
        # 优先从本地路径加载 feature_extractor，避免网络下载
        self.feature_extractor = CedFeatureExtractor.from_pretrained(model_name_or_path, trust_remote_code=True)
        full_model = CedForAudioClassification.from_pretrained(model_name_or_path, trust_remote_code=True)
        self.audio_encoder = full_model.encoder
        self.audio_encoder.eval()

    def forward(self, audio_tensor: torch.Tensor, sampling_rate: int = 16000):
        # 确保在无梯度环境下运行
        with torch.no_grad():
            # 用参数推断 device；不要依赖 .device 属性
            try:
            ...
            if audio_tensor.device.type != 'cpu':
                ...
                audio_tensor_cpu = audio_tensor

            # 2. 在 CPU 上提取梅尔频谱图
            # CedFeatureExtractor 内部会处理 numpy/tensor 转换
            features = self.feature_extractor(
                audio_tensor_cpu, 
                sampling_rate=sampling_rate, 
                return_tensors="pt"
            )
            # `features` 是一个 BatchFeature 字典，其值为 CPU 上的 tensor
            input_values_cpu = features.input_values

            # --- 模型推理阶段 (在目标设备上执行) ---
            # 3. 将提取好的特征张量移动到模型所在的设备
            input_values_device = input_values_cpu.to(model_device)

            # 4. 在目标设备上运行模型前向传播

            input_values_device = input_values_device.to(dtype=torch.float32)
            output = self.audio_encoder(input_values=input_values_device, output_hidden_states=True)
...
if __name__ == "__main__":
    import sys
    # python -m kimia_infer.models.tokenizer.ced_base.modeling_ced
    model_name_or_path = sys.argv[1] if len(sys.argv) > 1 else "/path/to/ced-base"
    model = CedEncoder(model_name_or_path)
    print("CED Model 权重名列表：")
