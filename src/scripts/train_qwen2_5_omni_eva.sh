#!/usr/bin/env bash
# Atomic training entry for Qwen2.5-Omni-EvA.
...
MAX_LENGTH=512
LEARNING_RATE=1e-3
TRAINABLE_MODULES=ced_processor
USE_LORA=False
LORA_R=64
LORA_ALPHA=64
LORA_DROPOUT=0.05
...
Required:
  --model, -m PATH              Qwen2.5-Omni-EvA model directory.
  --data, -d PATH               Training JSONL path.
  --output, -o PATH             Output checkpoint directory.

Options:
  --ced-model PATH              CED model directory. Default: MODEL_PATH/ced-base
  ...
  --max-length N                Max sequence length. Default: $MAX_LENGTH
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ced-model) shift; CED_MODEL_PATH="${1:-}" ;;
...
if [[ -z "$CED_MODEL_PATH" ]]; then
  CED_MODEL_PATH="$MODEL_PATH/ced-base"
fi
if [[ -z "$LOG_FILE" ]]; then
...
[[ -d "$CED_MODEL_PATH" ]] || { echo "CED model path not found: $CED_MODEL_PATH"; exit 1; }
[[ -f "$DATA_PATH" ]] || { echo "Data path not found: $DATA_PATH"; exit 1; }

MODEL_PATH="$(cd "$MODEL_PATH" && pwd)"
CED_MODEL_PATH="$(cd "$CED_MODEL_PATH" && pwd)"
...
torchrun \
  ...
  training/train_qwen2_5_omni_eva.py \
  --model_name_or_path "$MODEL_PATH" \
  --ced_model_path "$CED_MODEL_PATH" \
  --data_path "$DATA_PATH" \
